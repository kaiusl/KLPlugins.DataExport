...
namespace KLPlugins.DataExport.ksBroadcastingNetwork
{
    class ACCUdpRemoteClient : IDisposable
    {
        ...
        internal ACCUdpRemoteClient(string ip, int port, string displayName, string connectionPassword, string commandPassword, int msRealtimeUpdateInterval)
        {
            ...
            DataExport.LogInfo("Requested broadcast connection");
        ...
        internal void Shutdown()
        {
            ShutdownAsync().ContinueWith(t =>
            {
                if (t.Exception?.InnerExceptions?.Any() == true)
                    DataExport.LogError($"Client shut down with {t.Exception.InnerExceptions.Count} errors");
                else
                    DataExport.LogInfo("Client shut down asynchronously");
        ...
        private void OnBroadcastConnectionStateChanged(int connectionId, bool connectionSuccess, bool isReadonly, string error)
        {
            ...
            {
                DataExport.LogInfo("Connected to broadcast client.");
                IsConnected = true;
            }
            else
            {
                DataExport.LogWarn($"Failed to connect to broadcast client. Err: {error}");
                MessageHandler.RequestConnection(_displayName, _connectionPassword, _msRealtimeUpdateInterval, _commandPassword);
            }
        }
    ...
    class ACCUdpRemoteClientConfig
    {
        class ACCBroadcastConfig
        {
            ...
            internal void Validate()
            {
                ...
                {
                    udpListenerPort = _defUdpListenerPort;
                    throw new Exception($"Set broadcasting udp port in '..\\Config\\broadcasting.json' is invalid. Must be between 1024 and 65535.");
                }
            }
        }
        ...
        public ACCUdpRemoteClientConfig(string ip, string displayName, int updateTime)
        {
            try
            {
                var configPath = @"C:\Users\kaius\Documents\Assetto Corsa Competizione\Config\broadcasting.json";
                _config = JsonConvert.DeserializeObject<ACCBroadcastConfig>(File.ReadAllText(configPath, Encoding.Unicode).Replace("\"", "'"));
                _config.Validate();
            }
            catch (Exception e)
            {
                DataExport.LogWarn($"Couldn't read broadcasting.json because {e}. Using default.");
